...
def MovetoNew():
    ...
    try:
        originalComp = sel.Items[0].GetAncestor[IComponent]()
    except Exception:
        originalComp = None
    cyls = getTorusfromSur(sel)
    sel = Selection.GetActive()
    body = sel.Items[0]
    original_body_name = body.GetName()
    body.SetName("TO_BE_DELETED")
    DocumentHelper.CreateNewDocument()
    Copy.Execute(sel)
    GetRootPart().GetBodies()[0].SetName(original_body_name)
...
def getplanes(body):
    ...
    for i in facelist:
        ...
        if (
            i.Shape.Geometry.GetType() == Torus
            or i.Shape.Geometry.GetType() == ProceduralSurface
        ):
            torus.append(i)
...
def createcyl(lines, rad):
    ...
    TubeBody.Create(selection, MM(rad), ExtrudeType.None)
...
def fill():
    for face in GetRootPart().Bodies[0].Faces:
        if face.Shape.Geometry.GetType() == Sphere:
            ...
            options = FillOptions()
            result = Fill.Execute(
                selection, secondarySelection, options, FillMode.ThreeD, None
            )
...
def getarcfromtorus():
    ...
    plane1 = planelist[0]
    plane2 = planelist[1]
    p1 = plane1.EvalMid().Point
    P1 = SketchPoint.Create(p1)
    p2 = plane2.EvalMid().Point
    P2 = SketchPoint.Create(p2)
    p3 = torus[0].EvalMid().Point
    P3 = SketchPoint.Create(p3)
    sel = Selection.Create(
        [P1.CreatedCurves[0], P2.CreatedCurves[0], P3.CreatedCurves[0]]
    )
    ...
    seldel = Selection.Create(GetRootPart().Curves[:-1])
...
def getsolidcyls():
    ...
    if (
        len(cyls) > 2
    ):  # If the cylinders are pipes, delete also the inner cylinders and planes
        for c in cyls:
    ...
    options = (
        FixMissingFacesOptions()
    )  # Run it again because sometimes it doesnt find all the faces the first time
    FixMissingFaces.FindAndFix(options)
...
def getCorner():
    for bod in (
        GetRootPart().Bodies
    ):  # Extrude the correct end of the cylinder to an extremely long distance, this
...
def reduceCyl(thick):
    if thick > 1e-10:
        ...
        options.ExtrudeType = ExtrudeType.ForceIndependent
        result = OffsetFaces.Execute(
            selection, MM(-thick), Direction.Create(-0.7, 0.7, 0), options
        )
...
def goback(originalComp):
    ...
    bod = sorted(bods)[0][1]
    sel = Selection.Create(bod)
    DocumentHelper.CloseDocument()
    if originalComp:
        ComponentHelper.SetActive(originalComp)
    Copy.Execute(sel)
    if originalComp:
        result = originalComp.GetBodies()[-1]
    else:
        result = GetRootPart().GetBodies()[-1]
...
if cylNum > 1:
    ...
    seldel = Selection.Create(
        GetRootPart().Curves[0]
    )  # Delete the arc now that we have the lines
    Delete.Execute(seldel)
    lines = getlines()
    ...
    createPipe2(thick)
    cutExtraEnds()
    body = backToOriginal(originalComp)
...
# Deletes the original torus body
try:
    comp = body.GetAncestor[IComponent]()
except Exception:
    comp = None
if comp:
    for bod in comp.GetBodies():
        if bod.GetName() == "TO_BE_DELETED":
            seldel = Selection.Create(bod)
            break
else:
    for bod in GetRootPart().GetBodies():
        if bod.GetName() == "TO_BE_DELETED":
            seldel = Selection.Create(bod)
            break
Delete.Execute(seldel)  # Deletes the temporary body
